...
const Messages = (props) => {
  ...
  return props.conservant ? (
      ...
      {messages && conversation && !loading ? (
        <>
          <HorizontalStack
            alignItems="center"
            spacing={2}
            sx={{ px: 2, height: "60px", borderBottom: '1px solid #03A9FC', backgroundColor: '#00001E', boxShadow: '0px 0px 10px 0px #03A9FC'}}
          >
            {props.mobile && (
              <IconButton
                onClick={() => props.setConservant(null)}
                sx={{  padding: 0, color:" white", fontSize: "12px"}}
              >
                <AiFillCaretLeft />
                <span style={{
                  marginLeft: "5px"
            ...
            />
            <Typography>
              <Link style={{color: "yellow"}} to={"/users/" + props.conservant.username}>
                <b>{props.conservant.username}</b>
              </Link>
            </Typography>
          </HorizontalStack>
          <Divider />
          <Box sx={{ height: "calc(100vh - 240px)", padding: "20px"}}>
            <Box sx={{padding: "5px",  backgroundColor: '#00001E', height: "100%", borderRadius: "0.5rem", border: '1px solid #19A4F6' }}>
              <Stack
    ...
    <Stack
      sx={{ height: "100%", background: 'rgb(0,93,186)',
      background: 'radial-gradient(circle, rgba(0,93,186,1) 0%, rgba(0,0,30,0) 100%)'}}
      justifyContent="center"
      alignItems="center"
      spacing={2}
    >
      <AiFillMessage color='black' size={80} />
      <Typography style={{color: 'white'}} variant="h5">Los Pollos Messenger</Typography>
      <Typography color="white">
      Chat privately with other Pollos members!
      </Typography>
    </Stack>
  );
};
